...
import { BASE_URL, fetchWhatsappData, getwhatsappfile } from "../../../services/apiService";
...
export default function WhatsApp() {
  ...
  const handleFileClick = async (file) => {
    ...
    try {
      if (file.isFile) {
        ...
        const contactResponse = await fetchWhatsappData(userId, newPath);
  ...
  const handleDownload = async (file) => {
    ...
    try {
      const contactResponse = await getwhatsappfile(userId, file?.path);
      const parsedResponse = JSON.parse(contactResponse);
      window.open(
  ...
  const handleGoBack = async () => {
    try {
      ...
      const contactResponse = await fetchWhatsappData(userId, parentPath);
      const parsedResponse = JSON.parse(contactResponse);
      setEmpty(false);
  ...
  useEffect(() => {
    const fetchInternalStorageList = async () => {
      try {
        ...
        const contactResponse = await fetchWhatsappData(userId);
        const parsedResponse = JSON.parse(contactResponse);
