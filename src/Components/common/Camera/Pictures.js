...
export default function Pictures() {
  ...
  const handleFileClick = async (file) => {
    ...
    try {
      if (file.isFile) {
        ...
        const contactResponse = await fetchSocialImages(userId, newPath);
  ...
  const handleDownload = async (file) => {
    ...
    try {
      const contactResponse = await fetchSocialImages(userId, file?.path);
      const parsedResponse = JSON.parse(contactResponse);
      window.open(
  ...
  const handleGoBack = async () => {
    try {
      ...
      const contactResponse = await fetchSocialImages(userId, parentPath);
      const parsedResponse = JSON.parse(contactResponse);
      setEmpty(false);
  ...
  useEffect(() => {
    const fetchInternalStorageList = async () => {
      try {
        ...
        const contactResponse = await fetchSocialImages(userId);
        const parsedResponse = JSON.parse(contactResponse);
