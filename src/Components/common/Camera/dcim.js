...
import { BASE_URL, fetchDcimImages, getdcimfile } from "../../../services/apiService";
...
export default function DCIM() {
  ...
  const handleFileClick = async (file) => {
    ...
    try {
      if (file.isFile) {
        ...
        const contactResponse = await fetchDcimImages(userId, newPath);
  ...
  const handleDownload = async (file) => {
    ...
    try {
      const contactResponse = await getdcimfile(userId, file?.path);
      const parsedResponse = JSON.parse(contactResponse);
      window.open(
  ...
  const handleGoBack = async () => {
    try {
      ...
      const contactResponse = await fetchDcimImages(userId, parentPath);
      const parsedResponse = JSON.parse(contactResponse);
      setEmpty(false);
  ...
  useEffect(() => {
    const fetchInternalStorageList = async () => {
      try {
        ...
        const contactResponse = await fetchDcimImages(userId);
        const parsedResponse = JSON.parse(contactResponse);
